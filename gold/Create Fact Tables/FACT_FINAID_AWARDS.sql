CREATE OR REPLACE TABLE DITTEAU_DATA.GOLD.FACT_FINANCIAL_AID_AWARDS (
    ...
    DIM_AWARD_KEY INT,
    -- Award Data
    FIN_AID_AWARD_STATUS VARCHAR,
    FIN_AID_DISBURSEMENT_STATUS VARCHAR,
...
INSERT INTO DITTEAU_DATA.GOLD.FACT_FINANCIAL_AID_AWARDS (
    ...
    FIN_AID_TERM_KEY,
    DIM_AWARD_KEY,
    FIN_AID_AWARD_STATUS,
    FIN_AID_DISBURSEMENT_STATUS,
...
SELECT
    ...
    a.AWARD_KEY,
    aw.AWARD_STATUS,
    aw.DISBURSEMENT_STATUS,
    aw.AWARD_AMOUNT_OFFERED,
    aw.AWARD_AMOUNT_ACCEPTED,
    COALESCE(aw.DISBURSEMENT_AMOUNT_1,0) + COALESCE(aw.DISBURSEMENT_AMOUNT_2,0),
    d1.DATE_KEY,
    d2.DATE_KEY,
    'Y',
    aw.CREATION_DATE,
    ...
    CURRENT_TIMESTAMP(),
    aw.LAST_UPDATE_DATE
FROM DITTEAU_DATA.SILVER.SILVER_PF_AWARDS aw
JOIN DITTEAU_DATA.SILVER.SILVER_PF_APPLICATIONS app
    ON aw.STUDENT_ID = app.STUDENT_ID
JOIN DITTEAU_DATA.GOLD.DIM_AWARDS a
    ON a.AWARD_CODE = aw.award_code
LEFT JOIN DITTEAU_DATA.GOLD.DIM_INTEGRATION_IDS i  -- changed this to left 8/13 why wasn't it finding it??
    ON aw.STUDENT_ID = i.POWERFAIDS_STUDENT_ID
LEFT JOIN DITTEAU_DATA.GOLD.DIM_STUDENT s
    ON s.STUDENT_KEY = i.DD_STUDENT_ID
LEFT JOIN DITTEAU_DATA.GOLD.DIM_TERM t
    ON aw.AWARD_YEAR = t.term_acad_yr
LEFT JOIN DITTEAU_DATA.GOLD.DIM_DATE d1
    ON aw.DISBURSEMENT_DATE = d1.DATE_ACTUAL
LEFT JOIN DITTEAU_DATA.GOLD.DIM_DATE d2
    ON aw.DISBURSEMENT_DATE_SCHEDULED_2 = d2.DATE_ACTUAL;
